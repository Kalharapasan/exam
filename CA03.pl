% Tree structure (edges as parent-child relations)
child(a, b).
child(a, c).
child(a, d).
child(b, e).
...
bfs([Head|Tail], [Head|Result]) :-
    children(Head, Children),
    append(Tail, Children, NewQueue),
    bfs(NewQueue, Result).

% DFS helper
dfs([], []).

dfs([Head|Tail], [Head|Result]) :-
    children(Head, Children),
    append(Children, Tail, NewStack),
    dfs(NewStack, Result).
...
% ----------- TREE STRUCTURE FOR BFS & DFS -----------

% Tree structure using child/2 facts
child(a, b). child(a, c). child(a, d).
child(b, e). child(b, f).
child(c, g). child(c, h). child(c, i).
child(d, j).
child(e, k). child(e, l).
child(f, m).
child(h, n).
child(i, o). child(i, p).
child(j, q). child(j, r). child(j, s).
child(m, t).
child(n, u).
child(o, v).
...
% DFS traversal
dfs([], []).
dfs([Node|Rest], [Node|Traversal]) :-
    findall(Child, child(Node, Child), Children),
    append(Children, Rest, Stack),
    dfs(Stack, Traversal).

% ----------- MISSIONARIES AND CANNIBALS PROBLEM -----------

% Valid state: (ML, CL, BoatPos, MR, CR)
valid_state(state(ML, CL, left, MR, CR)) :-
    ML >= 0, CL >= 0, MR >= 0, CR >= 0,
    (ML >= CL ; ML == 0),
    (MR >= CR ; MR == 0).

valid_state(state(ML, CL, right, MR, CR)) :-
    ML >= 0, CL >= 0, MR >= 0, CR >= 0,
    (ML >= CL ; ML == 0),
    (MR >= CR ; MR == 0).
...
move(state(ML, CL, left, MR, CR), state(ML2, CL2, right, MR2, CR2)) :-
    ...
    valid_state(state(ML2, CL2, right, MR2, CR2)).

move(state(ML, CL, right, MR, CR), state(ML2, CL2, left, MR2, CR2)) :-
    member([M, C], [[2,0], [0,2], [1,1], [1,0], [0,1]]),
    ML2 is ML + M, CL2 is CL + C,
    MR2 is MR - M, CR2 is CR - C,
    valid_state(state(ML2, CL2, left, MR2, CR2)).
...
missionaries_and_cannibals(Steps) :-
